...
def plot_some(test_data, colorization_model,epoch):
  with torch.no_grad():
    indexes = [0, 2, 9]
    for idx in indexes:
    # for batch in range(TOTAL_TEST_BATCH):
      ...
      batchL_3 = torch.tensor(np.tile(batchL, [1, 3, 1, 1])).float()
      batchL = torch.tensor(batchL).float()
      realAB = torch.tensor(realAB).float()
